...
const Footer = () => {
  ...
  const handleNewsletterSubmit = async (e) => {
    ...
    try {
      ...
      if (Math.random() > 0.1) { // Simula sucesso
        ...
        throw new Error("Falha simulada na inscrição da newsletter.");
      }
    } catch (error) {
  ...
  const socialLinks = [
    ...
    { icon: Instagram, href: "https://instagram.com/engbras", label: "Instagram" },
    { icon: Linkedin, href: "https://linkedin.com/company/engbras", label: "LinkedIn" },
    ...
    <footer className="bg-card border-t border-border/70 text-foreground/80 pt-16 pb-8">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          >
            <Link to="/" className="text-3xl font-slab font-bold text-primary mb-4 inline-block">
              Engbras
            </Link>
            <p className="text-sm leading-relaxed mb-6">
              Inovação e excelência em engenharia para construir o futuro. Comprometidos com a qualidade e a satisfação de nossos clientes.
            </p>
          ...
          >
            <p className="font-semibold text-foreground mb-4 text-lg">Navegação</p>
            <ul className="space-y-2">
              {footerLinks.slice(0, 5).map((link) => (
                <li key={link.to}>
                  <Link to={link.to} className="hover:text-primary transition-colors duration-200 text-sm">
                    {link.text}
                  </Link>
                </li>
              ))}
          ...
          >
            <p className="font-semibold text-foreground mb-4 text-lg">Newsletter</p>
            <p className="text-sm mb-3">Receba nossas últimas notícias e insights diretamente no seu email.</p>
        ...
        >
          <p>&copy; {currentYear} Engbras Engenharia e Inovação. Todos os direitos reservados.</p>
