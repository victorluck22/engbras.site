...
import { Menu, X, Briefcase, Home, Newspaper, Info, MessageSquare, Users, PlusSquare, Sun, Moon, LayoutDashboard } from "lucide-react";
...
const Header = () => {
  ...
  const isActiveLink = (linkPath) => {
    if (linkPath.includes("#")) {
      return location.pathname === "/" && location.hash === linkPath.substring(linkPath.indexOf("#"));
    }
    return location.pathname === linkPath || (linkPath !== "/" && location.pathname.startsWith(linkPath));
  };
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <nav className="hidden lg:flex items-center space-x-1">
            {navLinks.map((link) => (
             ...
             <Button asChild size="sm" className="bg-primary hover:bg-primary/90 text-primary-foreground ml-4 transform hover:scale-105 transition-transform duration-200">
               <Link to="/admin/dashboard" className="flex items-center"><LayoutDashboard size={16} className="mr-2"/>Painel Admin</Link>
            </Button>
            <Button
              variant="ghost"
              size="icon"
            ...
            >
              <AnimatePresence initial={false} mode="wait">
                <motion.div
                  ...
                  initial={{ opacity: 0, rotate: mobileMenuOpen ? -90 : 90, scale: 0.5 }}
                  animate={{ opacity: 1, rotate: 0, scale: 1 }}
                  exit={{ opacity: 0, rotate: mobileMenuOpen ? 90 : -90, scale: 0.5 }}
        ...
        {mobileMenuOpen && (
          ...
          >
            <div className="container mx-auto px-4 py-4 space-y-1">
              ...
              <Button asChild className="w-full mt-3 bg-primary hover:bg-primary/90 text-primary-foreground py-3 text-base flex items-center">
                 <Link to="/admin/dashboard"><LayoutDashboard size={18} className="mr-2"/>Painel Admin</Link>
              </Button>
            </div>
          </motion.div>
        )}
