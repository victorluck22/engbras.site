    ...
    <div className="relative w-full aspect-video rounded-lg overflow-hidden shadow-xl mb-8 group">
      <Dialog>
       ...
       <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
        <p className="text-white text-sm">Imagem {currentIndex + 1} de {images.length}</p>
      </div>
...
const ProjectDetailPage = () => {
  ...
  useEffect(() => {
    const storedProjects = JSON.parse(localStorage.getItem("engbrasProjects") || "[]");
    ...
    setTimeout(() => {
        if (currentProject) {
            ...
            const detailedData = {
                ...currentProject,
                images: currentProject.images || [ // Add default images if none exist
                    currentProject.imageUrl || "https://images.unsplash.com/photo-1519681393784-d120267933ba?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1170&q=80",
  ...
  const projectImage = project.images && project.images.length > 0 ? project.images[0] : `${siteUrlBase}/og-image-default-project.jpg`;
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="keywords" content={`Engbras, projeto ${project.title}, ${project.category}, engenharia, ${project.title}`} />
        ...
        <meta property="og:image:alt" content={`Imagem do projeto ${project.title}`} />
      </Helmet>
      <motion.div
        initial="initial"
        animate="in"
      ...
      >
        <article className="max-w-4xl mx-auto bg-engbras-white p-6 sm:p-10 rounded-xl shadow-2xl border border-engbras-gray/40">
          <motion.div 
            ...
            animate={{ opacity: 1, x: 0, transition: { delay: 0.1, duration: 0.6 } }}
            className="mb-8"
          >
            <Button asChild variant="outline" className="border-engbras-orange text-engbras-orange hover:bg-engbras-orange hover:text-engbras-white group transition-all duration-300">
              <Link to="/projects">
                <ArrowLeft className="mr-2 h-4 w-4 group-hover:-translate-x-1 transition-transform" /> Voltar para Projetos
              </Link>
            </Button>
          </motion.div>
          ...
          <motion.h1 
            ...
            className="text-3xl sm:text-4xl lg:text-5xl font-bold text-engbras-gray-dark mb-3"
          ...
          <motion.div 
            ...
            animate={{ opacity:1, scale:1, transition:{ delay: 0.35, duration:0.7, ease:[0.6, 0.01, -0.05, 0.95]} }}
          >
