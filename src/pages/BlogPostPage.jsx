...
const BlogPostPage = () => {
  ...
  const pageDescription = post.summary || post.content.substring(0, 160).replace(/<[^>]*>?/gm, '') + "..."; // Remove HTML for description
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="keywords" content={`Engbras, engenharia, ${post.title}, ${post.tags ? post.tags.join(", ") : "blog"}`} />
        ...
        <article className="max-w-3xl mx-auto bg-engbras-white p-6 sm:p-10 rounded-xl shadow-2xl border border-engbras-gray">
          {post.imageUrl && (
            <motion.img 
              ...
              animate={{ opacity: 1, height: 'auto', transition: { duration: 0.7, ease: "easeOut" } }}
              className="w-full max-h-[400px] object-cover rounded-lg mb-8 shadow-md"
              alt={post.title || "Imagem de capa do post"}
              src={post.imageUrl || "https://images.unsplash.com/photo-1697256200022-f61abccad430"}
            />
          )}
          <motion.h1 
          ...
          >
            <Button asChild variant="outline" className="border-engbras-orange text-engbras-orange hover:bg-engbras-orange hover:text-engbras-white group transition-all duration-300">
              <Link to="/blog">
                <ArrowLeft className="mr-2 h-4 w-4 group-hover:-translate-x-1 transition-transform" /> Voltar para o Blog
              </Link>
            </Button>
          </motion.div>
        </article>
